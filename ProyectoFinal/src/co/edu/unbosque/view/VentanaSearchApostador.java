...
public class VentanaSearchApostador extends JFrame{
	private JButton ingresar, regresar;
	private JTextField cedula;
	private JLabel imagenFondo, action;

	public VentanaSearchApostador(String accion) {
		...
		cedula.setBounds(170, 320, 328, 60); 
		cedula.setToolTipText("Ingresa la cedula"); 
		add(cedula);
		
		action = new JLabel();
		action.setText(accion);
		action.setFont(new Font("@Yu Gothic UI Semibold", Font.PLAIN, 18)); 
		action.setForeground(Color.WHITE);
		action.setBounds(428, 170, 328, 60);
		add(action);
		
		imagenFondo = new JLabel();
		imagenFondo.setBounds(0, 0, getWidth(), getHeight());
		Image temp;
		temp = new ImageIcon("src/Imagenes/SearchApostador.png").getImage();
		ImageIcon img;
		img = new ImageIcon(
